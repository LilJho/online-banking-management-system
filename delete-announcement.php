...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['id'])) {
        $announcementId = intval($_POST['id']); // Get the offerId and cast it to an integer
        ...
        if ($stmt->execute()) {
            if ($stmt->affected_rows > 0) {
                ...
                echo json_encode(['error' => 'No Announcement found with the given ID']);
            }
        } else {
            error_log('Error executing statement: ' . $stmt->error);
            echo json_encode(['error' => 'Error deleting the Announcement']);
        }
